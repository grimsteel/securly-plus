...
const db = openDB("__securly-plus-db", 2, {
...
/** @type {{ defaultScheduleTab: string, defaultScreen: string, idbUrl: string, sessionCaching: boolean, instantRequests: boolean, ignoreLimits: boolean, bulkFlexing: boolean, startDirectoryToday: boolean, sourcemap: null | { foundNames: Record<string, {chunkId: number, exportName: string}> } } | null} */
let data = null;
...
// evict LRU cache
(async () => {
  const stores = ["schedulings-cache", "catchall-cache"];
  const tx = (await db).transaction(stores, "readwrite");
  const fifteenDaysAgo = Date.now() - 15 * 24 * 3600 * 1000;
  const range = IDBKeyRange.upperBound(fifteenDaysAgo, true);

  for (const storeName of stores) {
    const store = tx.objectStore(storeName);
    // remove > 15 days ago
    const index = store.index("last-accessed");
            ...
            reqObservable.subscribe(async r => {
              ...
              if (r.type === 4) {
                ...
                if (item && mostRecentActivityScheduleId) {
                  ...
                  const tx = (await db).transaction(["schedulings-cache", "catchall-cache"], "readwrite");
          ...
          return new Observable(observer => {
            ...
            const fetchRequest = fetch(url, {
              ...
              .then(async r => {
                // update the cache
                try {
                  const tx = (await db).transaction(["schedule-item-cache", "catchall-cache", "schedulings-cache"], "readwrite");
            ...
            const cacheRequest = (async () => {
              const tx = (await db).transaction(["schedule-item-cache", "catchall-cache", "schedulings-cache"], "readonly");
...
let patchedCount = 0;
let total = Object.keys(chunks).length;
// we only need to patch webpackRequire.d once
let patchedRequire = false;
// chunks are identified in the require.d by this symbol
...
window.webpackChunkeduspire = new Proxy(array, {
  ...
  get(_target, prop, _receiver) {
    if (prop === "push" && webpackPush) {
      return item => {
        // go through all chunks pushed here
        for (const [chunkId, v] of Object.entries(item[1])) {
          // check if we care about this chunk
          if (!Object.values(data?.sourcemap?.foundNames ?? {}).some(n => n.chunkId == chunkId)) continue;

          // monkey-patch chunk function
          item[1][chunkId] = (...args) => {
            // intercept the chunk export registry

            // save the symbol for this chunk
            chunkSymbols.set(args[1], chunkId);

            if (patchedRequire) return v(...args);
            patchedRequire = true;

            const webpackRequire = args[2];
            // harmony exports define property getters
            const [dKey, dFun] = Object.entries(webpackRequire)
            // 2-param function with "defineProperty"
                  .find(([_k, v]) => v.length === 2 && v.toString().includes("defineProperty"));

            log("webpatcher", "patched __webpack_require__.d");
            
            // patch the export function
            webpackRequire[dKey] = (...dFunctionArgs) => {
              // when it's called, the config function passes in another function as the second argument
              // monkey-patch it, but identify which chunk this is
              const chunkId = chunkSymbols.get(dFunctionArgs[0]);
              if (chunkId) {
                // iterate through all exports relevant for this chunk
                for (const [name, { exportName }] of Object.entries(data?.sourcemap?.foundNames || {}).filter(([_k, v]) => v.chunkId == chunkId)) {
                  const origXFunction = dFunctionArgs[1][exportName];
                  let hasHooked = false;
                  let overriddenResult = null;
                  dFunctionArgs[1][exportName] = () => {
                    // this function is the one we care about
                    
                    const result = origXFunction();
                    if (!hasHooked) {
                      hasHooked = true;
                      log("webpatcher", `patched ${name} (${(++patchedCount)}/${total})`);
                      
                      const overrideResult = chunks[name](result);
